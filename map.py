...
# Load dataset
file_path = "E:\Learn\OSU\DataIO\02082025\archive\ev_stations_v1.csv"
...
m_main = Basemap(
    projection='merc',
    llcrnrlat=24, urcrnrlat=50,  # Latitude range (Mainland USA)
...
for charger, color in color_mapping.items():
    ...
    ax_main.scatter(subset_public["x_main"], subset_public["y_main"], s=10, color=color, alpha=0.7, marker='o')
    ax_main.scatter(subset_private["x_main"], subset_private["y_main"], s=40, color=color, alpha=0.7, marker='*')

# **Move Alaska and Hawaii to the bottom-left (standard USA map layout)**
ax_alaska = fig.add_axes([0.07, 0.08, 0.22, 0.22])  # Bottom-left
ax_hawaii = fig.add_axes([0.18, 0.05, 0.18, 0.18])  # Below Alaska
...
for charger, color in color_mapping.items():
    ...
    ax_alaska.scatter(subset_public["x_alaska"], subset_public["y_alaska"], s=10, color=color, alpha=0.7, marker='o')
    ax_alaska.scatter(subset_private["x_alaska"], subset_private["y_alaska"], s=40, color=color, alpha=0.7, marker='*')

ax_alaska.set_title("Alaska (AK)", fontsize=10)
...
for charger, color in color_mapping.items():
    ...
    ax_hawaii.scatter(subset_public["x_hawaii"], subset_public["y_hawaii"], s=10, color=color, alpha=0.7, marker='o')
    ax_hawaii.scatter(subset_private["x_hawaii"], subset_private["y_hawaii"], s=40, color=color, alpha=0.7, marker='*')

ax_hawaii.set_title("Hawaii (HI)", fontsize=10)

# **Move the legend (charging type table) to the bottom-right**
legend_ax = fig.add_axes([0.75, 0.05, 0.2, 0.2])
legend_ax.axis("off")  # Hide the axes

# Legend content
legend_ax.text(0.1, 0.9, "Charger Type & Access Type", fontsize=12, fontweight="bold")
